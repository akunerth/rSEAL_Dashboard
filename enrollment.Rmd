...
library(haven)
Consents2 <- data.frame(read_sav(file = '~/Dropbox (University of Oregon)/SEAL R01/_Remote SEAL/rSEAL_Data/Consent_Tracker.sav'))
...
CaseProg <- data.frame(read_sav(file = '~/Dropbox (University of Oregon)/SEAL R01/_Remote SEAL/rSEAL_Data/Case_Progression_Log.sav'))

#Read in Coach Impressions
CoachImp <- data.frame(read_sav(file = '~/Dropbox (University of Oregon)/SEAL R01/_Remote SEAL/rSEAL_Data/Coach_Impressions_Form.sav'))
...
CaseProg$Q10 <- as.Date(CaseProg$Q10,"%Y-%m-%d")
...
Consents2.2 <- Consents2.1[match(unique(Consents2.1$Informed.Consent), Consents2.1$Informed.Consent),]
...
#Remove duplicate response from RS018
EnrollmentConsents <- EnrollmentConsents %>% slice(-c(48))
EnrollmentConsents <- full_join(EnrollmentConsents,CoachImp)
#Remove duplicate response from RS039
EnrollmentConsents <- EnrollmentConsents %>% slice(-c(29))
...
Test4.1 <- today - Enrollment$LENAD1DiaryActual
Test5 <- today - Enrollment$LENA2Expected
Test6 <- today - Enrollment$CoachImpComplete
...
Test11.1 <- today - Enrollment$LENAD4DiaryActual
...
Wv1Comp<- (Enrollment13.1$ConsentStatus == "Complete" & Enrollment13.1$LENAD2DiaryStatus == "Complete")
Wv1Comp <- length(grep("TRUE", Wv1Comp))
Wv1DropOut <- (Enrollment13.1$DropOutStage == 1)
Wv1DropOut <- length(grep("TRUE", Wv1DropOut))
Wv1InProg<- ((Enrollment13.1$LENAD2DiaryStatus == "Delinquent" | Enrollment13.1$LENAD2DiaryStatus == "Warning" | Enrollment13.1$LENAD2DiaryStatus == "On-Track" | Enrollment13.1$LENAD2DiaryStatus == "NA") & Enrollment13.1$ConsentStatus == "Complete") 
Wv1InProg <- (length(grep("TRUE", Wv1InProg)) - Wv1DropOut)
...
IntvInProg<- (is.na(Enrollment13.1$CoachImpComplete) & (Enrollment13.1$LENAD2DiaryStatus == "Complete" | Enrollment13.1$LENAD2DiaryStatus =="Opt-Out") & Enrollment13.1$Surv3Status !="Opt-Out")
IntvInProg <- (length(grep("TRUE", IntvInProg)) - IntvDropOut)
Wv2Comp<- ((Enrollment13.1$LENAD4DiaryStatus== "Complete"| Enrollment13.1$LENAD4DiaryStatus == "Opt-Out")& (is.na(Enrollment13.1$DropOutDate)))
Wv2Comp <- length(grep("TRUE", Wv2Comp))
Wv2DropOut <- (Enrollment13.1$DropOutStage == 3)
Wv2DropOut <- length(grep("TRUE", Wv2DropOut))
Wv2InProg<- (Enrollment13.1$CoachImpComplete > "2020-07-01" & (Enrollment13.1$LENAD4DiaryStatus == "Delinquent" | Enrollment13.1$LENAD4DataStatus == "On-Track" | Enrollment13.1$LENAD4DiaryStatus == "Warning" | Enrollment13.1$LENAD4DiaryStatus == "NA")) 
Wv2InProg <- (length(grep("TRUE", Wv2InProg)) - Wv2DropOut)
...
scale_x_discrete(labels=c("Consent","PostConsentStatus", "Survey 1", "Survey 2", "PDR2", "PDR3", "LENA D1 Diary", "LENA D1 Data","LENA D2 Diary", "LENA D2 Data", "Survey 3", "Survey 4", "PDR5", "PDR6", "LENA D3 Diary", "LENA D3 Data", "LENA D4 Diary", "LENA D4 Data")) +
scale_fill_manual(values = c("#bdbdbd","#e31a1c","#1f78b4","#737373","#33a02c" #"#ffff99"
                             ), labels = c("NA", "Delinquent","On-Track",   #"Warning",  
                                            "Opt-Out","Complete")) #Manually assigns colors
