...
Consents1$Informed.consent <- as.Date(Consents1$Informed.consent , "%Y/%m/%d")
Consents2$Informed.consent <- as.Date(Consents2$Informed.consent , "%Y/%m/%d")
Consents1$Staff.Signature <- as.Date(Consents1$Staff.Signature , "%Y/%m/%d")
...
Consents1 <- data.frame("RSID" = Consents1$RSID,
                        "InformedConsent"=Consents1$Informed.consent, "Recontact" = Consents1$Recontact, "StaffSignature"=Consents1$Staff.Signature)

Consents2 <- data.frame("RSID" = Consents2$RSID,
                        "InformedConsent"=Consents2$Informed.consent, "Recontact" = Consents2$Recontact, "StaffSignature"=Consents2$Staff.Signature)
...
Enrollment3 <- Enrollment2%>%
      ...
      Test > 3 & is.na(Survey2Actual) ~ "Delinquent",
      Test < 3 & is.na(Survey2Actual) ~ "On-Track",
...
scale_fill_discrete(labels = c("Complete", "Delayed", "Delinquent")) #Manually assigns colors
...
Wave1Counts <- kable(Wave1,
        col.names = c("Complete","Delayed","Delinquent")) %>%
        kable_minimal()
Wave1Counts
```
:::