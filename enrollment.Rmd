...
author: 
date: "Last Updated: 2/24/2021, 11:35pm"
output: 
  html_document:
    theme: flatly
    toc: TRUE
...
#Read Masterfile
library(haven)
masterfile <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/masterfile.sav'))
LENADataW1 <- data.frame(read_xlsx("/Volumes/psy-ctn/psy-ctn/SNAPLab/SNAP Projects/SEAL/00_rSEAL/01_Data_Management/LENAcomposite.xlsx", sheet = "W1"))
LENADataW2 <- data.frame(read_xlsx("/Volumes/psy-ctn/psy-ctn/SNAPLab/SNAP Projects/SEAL/00_rSEAL/01_Data_Management/LENAcomposite.xlsx", sheet = "W2"))
#rSEAL Manual Consent Data
Consents1 <- data.frame(read_xlsx('~/Box/SEAL-r/rSEAL_Data/rSEAL_manual_consents.xlsx', 'Sheet1'))
#rSEAL Test Consent Data
library(haven)
Consents2 <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/Consent_Tracker.sav'))
Consents2 <- subset(Consents2, select = -c(2:17))
Consents2$recontact <- as.numeric(Consents2$recontact)
#Consents2 <- data.frame(read_xlsx('~/Dropbox (University of Oregon)/SEAL R01/_Remote SEAL/rSEAL_Data/Consent_Tracker.xlsx', 'Sheet1'))

#Read in LENA Tracker for Expected Dates
LENATracker <- data.frame(read.csv('~/Box/SEAL-r/rSEAL_Data/FamTracker Data/LENA_Data_w1.csv'))
#Read in Case Progression Log
CaseProg <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/Case_Progression_Log.sav'))

#Read in Fidelity Data
library(haven)
FIND <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/FIND_Fidelity.sav'))
HTP <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/HTP_Fidelity.sav'))

#Read in Coach Impressions
CoachImp <- data.frame(read_sav(file = '~/Box/SEAL-r/rSEAL_Data/Coach_Impressions_Form.sav'))
...
CaseProg <- CaseProg %>% slice(-c(45))
#Remove duplicate response RS039 
CoachImp <- CoachImp %>% slice(-c(14))
#Classify Tests
#ConsentCompletion <- masterfile$ConsentCompletion <- ifelse(masterfile$SubInfoStart > #"2020-07-01", "Complete", "Incomplete")
...
#LENAD1DiaryCompletion <- masterfile$LENAD1DiaryCompletion <- #ifelse(masterfile$LENAD1Start > "2020-07-01", "Complete", "Incomplete")
#LENAD2DiaryCompletion <- masterfile$LENAD2DiaryCompletion<- ifelse(masterfile$LENAD2Start #> "2020-07-01", "Complete", "Incomplete")
...
#LENAD3DiaryCompletion <- masterfile$LENAD3DiaryCompletion <- #ifelse(masterfile$LENAD3Start > "2020-07-01", "Complete", "Incomplete")
#LENAD4DiaryCompletion <- masterfile$LENAD4DiaryCompletion <- #ifelse(masterfile$LENAD4Start > "2020-07-01", "Complete", "Incomplete")
#CoachImpCompletion <- CoachImp$CoachImpCompletion <- ifelse(CoachImp$RecordedDate > #"2020-07-01", "Complete", "Incomplete")
...
#Classify Tests
#Enrollment$ConsentCompletion <- ifelse(Enrollment$Informed.Consent > "2020-07-01", #"Complete", "Incomplete")
#Enrollment$LENAD1DataCompletion <- ifelse(Enrollment$LENAD1DataW1  > 0, "Complete", #"Incomplete")
#Enrollment$LENAD2DataCompletion <- ifelse(Enrollment$LENAD2DataW1 > 0, "Complete", #"Incomplete")
#Enrollment$LENAD3DataCompletion <- ifelse(Enrollment$LENAD1DataW2  > 0, "Complete", #"Incomplete")
#Enrollment$LENAD4DataCompletion <- ifelse(Enrollment$LENAD2DataW2   > 0, "Complete", #"Incomplete")
#Enrollment$IntvCompletion <- ifelse(Enrollment$CoachImpComplete > "2020-07-01", #"Complete", "Incomplete")
```
...
DropOutDate <- Enrollment$DropOutDate
...
Test6 <- today - as.Date(Enrollment$CoachImpComplete, "%y-%m-%d")
...
Test10 <- today - as.Date(Enrollment$PDR6Actual, "%y-%m-%d")
Test11 <- today - as.Date(Enrollment$LENAD3DiaryActual, "%y-%m-%d")
Test11.1 <- today - as.Date(Enrollment$LENAD4DiaryActual, "%y-%m-%d")
...
Enrollment1.1 <- Enrollment1 %>%
      ...
      DropOutDate > start & is.na(Enrollment$SubInfoActual) ~ "Opt-Out",
...
Enrollment6.1 <- Enrollment6%>%
      ...
      DropOutDate > start & is.na(LENAD1DataW1) ~ "Opt-Out",
      Enrollment6$LENAD1OptOut > start & is.na(LENAD1DataW1) ~ "Opt-Out",
      Test4.1 > 7 & is.na(LENAD1DataW1) ~ "Delinquent",
      Test4.1 <= 7 & Test4.1 >= 3 & is.na(LENAD1DataW1) ~ "Warning",
      Test4.1 < 3 & is.na(LENAD1DataW1) ~ "On-Track",
...
Enrollment7.1 <- Enrollment7%>%
      ...
      DropOutDate > start & is.na(LENAD2DataW1) ~ "Opt-Out",
      Enrollment7$LENAD2OptOut > start & is.na(LENAD2DataW1) ~ "Opt-Out",
      Test6 > 7 & is.na(LENAD2DataW1) ~ "Delinquent",
      Test6 <= 7 & Test4.1 >= 3 & is.na(LENAD2DataW1) ~ "Warning",
      Test6 < 3 & is.na(LENAD2DataW1) ~ "On-Track",
...
Enrollment12.1 <- Enrollment12%>%
      ...
      DropOutDate > start & is.na(LENAD1DataW2) ~ "Opt-Out",
      Enrollment12$LENAD3OptOut > start & is.na(LENAD1DataW2) ~ "Opt-Out",
      Test11 > 7 & is.na(LENAD1DataW2) ~ "Delinquent",
      Test11 <= 7 & Test4.1 >= 3 & is.na(LENAD1DataW2) ~ "Warning",
      Test11 < 3 & is.na(LENAD1DataW2) ~ "On-Track",
...
Enrollment13.1 <- Enrollment13%>%
      ...
      DropOutDate > start & is.na(LENAD2DataW2) ~ "Opt-Out",
      Enrollment13$LENAD4OptOut > start & is.na(LENAD2DataW2) ~ "Opt-Out",
      Test11.1 > 7 & is.na(LENAD2DataW2) ~ "Delinquent",
      Test11.1 <= 7 & Test11.1 >= 3 & is.na(LENAD2DataW2) ~ "Warning",
      Test11.1 < 3 & is.na(LENAD2DataW2) ~ "On-Track",
...
PostConsentComp<- (Enrollment13.1$ConsentStatus == "Complete" & Enrollment13.1$PostConsentStatus == "Complete")
PostConsentComp<- length(grep("TRUE", PostConsentComp))
PostConsentInProg<- (Enrollment13.1$ConsentStatus == "Complete" & Enrollment13.1$PostConsentStatus != "Complete")
...
Wv1Comp<-  ((Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "Complete") | (Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "Opt-Out"))
Wv1Comp <- (length(grep("TRUE", Wv1Comp)) - Wv1DropOut)
Wv1InProg<- ((Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "Delinquent") | (Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "Warning") | (Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "On-Track") | (Enrollment15$Instrument == "LENAD2DiaryStatus" & Enrollment15$Status == "NA"))
Wv1InProg <- length(grep("TRUE", Wv1InProg))
IntvComp<- (Enrollment13.1$CoachImpComplete > "2020-07-01")
IntvComp <- length(grep("TRUE", IntvComp))
IntvDropOut <- (Enrollment13.1$DropOutStage == 2)
IntvDropOut <- length(grep("TRUE", IntvDropOut))
IntvInProg<- (is.na(Enrollment13.1$CoachImpComplete) & Enrollment13.1$LENAD2DiaryStatus == "Complete" | Enrollment13.1$LENAD2DiaryStatus == "Opt-Out") 
IntvInProg <- (length(grep("TRUE", IntvInProg))-(Wv1DropOut+IntvDropOut))
Wv2DropOut <- (Enrollment13.1$DropOutStage == 3)
Wv2DropOut <- length(grep("TRUE", Wv2DropOut))
Wv2Comp<- ((Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "Complete") | (Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "Opt-Out"))
Wv2Comp <- (length(grep("TRUE", Wv2Comp)) - (Wv2DropOut + Wv1DropOut + IntvDropOut))
Wv2InProg<- ((Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "Delinquent") | (Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "Warning") | (Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "On-Track") | (Enrollment15$Instrument == "LENAD4DiaryStatus" & Enrollment15$Status == "NA") & (Enrollment15$Instrument == "Surv3Status" & Enrollment15$Status == "Complete"))
Wv2InProg <- length(grep("TRUE", Wv2InProg)-(Wv1DropOut+IntvDropOut))
