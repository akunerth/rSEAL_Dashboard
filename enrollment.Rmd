...
```{r SEAL New DataFrame, echo = FALSE, fig.width=5, fig.height=4, message=FALSE, warning=FALSE}
...
Enrollment2 <- Enrollment %>%
      ...
      Test < 7 & is.na(Survey1Actual) ~ "Green",
...
Enrollment3 <- Enrollment2%>%
      ...
      Test < 7 & is.na(Survey2Actual) ~ "Green",
      Survey2Actual > start       ~ "Complete",
      TRUE                      ~ "Yellow"
      )
     )

Enrollment4 <- gather(Enrollment3, "Instrument", "Status", 6:7)

Wave1 <- table(Enrollment4$Instrument, Enrollment4$Status, dnn=c("Instrument", "Status"))
...
Enrollment4$Cases <- factor(Enrollment4$RSID) # Create a categorical variable
Enrollment4$Instrument <- factor(Enrollment4$Instrument) # Create categorical variable
p <- ggplot(data = Enrollment4, aes(x=Enrollment4$Instrument, fill=Enrollment4$Status) ) + geom_bar() + # Creates stacked bar chart
scale_x_discrete(labels=c("Survey 1", "Survey 2")) +
scale_fill_discrete(labels = c("Complete", "Delinquent")) #Manually assigns colors
p <- p + xlab("Instrument") + ggtitle("Wave 1 Statuses") + labs(fill = "Statuses") # Adds title and labels
p
