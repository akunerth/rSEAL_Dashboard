...
EnrollmentConsents <- right_join(EnrollmentConsents,LENATracker)
library(dplyr)
...
<div class="btn-group btn-group-justified">
 <div class="btn-group">
<a href="#" class="btn btn-danger">**Total Complete**<br>TBD</a>
  </div>
 <div class="btn-group">
<a href="#" class="btn btn-danger">**Total In Progress**<br>TBD</a>
  </div>
...
Enrollment1.1 <- Enrollment1 %>%
      ...
      Survey1Actual > start & is.na(SubInfoActual) ~ "Missing",
...
Enrollment2 <- Enrollment1.1 %>%
      ...
      Survey2Actual > start & is.na(Survey1Actual) ~ "Missing",
...
Enrollment3 <- Enrollment2%>%
      ...
      PDR2Actual > start & is.na(Survey2Actual) ~ "Missing",
...
Enrollment4 <- Enrollment3%>%
      ...
      PDR3Actual > start & is.na(PDR2Actual) ~ "Missing",
...
Enrollment5 <- Enrollment4%>%
      ...
      LENAD1DiaryActual > start & is.na(PDR3Actual) ~ "Missing",
...
Enrollment6 <- Enrollment5%>%
      ...
      LENAD2DiaryActual > start & is.na(LENAD1DiaryActual) ~ "Missing",
...
Enrollment7 <- Enrollment6%>%
      ...
      Survey3Actual > start & is.na(LENAD2DiaryActual) ~ "Missing",
...
Enrollment8 <- Enrollment7%>%
      ...
      Survey4Actual > start & is.na(Survey3Actual) ~ "Missing",
...
Enrollment9 <- Enrollment8%>%
      ...
      PDR5Actual > start & is.na(Survey4Actual) ~ "Missing",
...
Enrollment10 <- Enrollment9%>%
      ...
      PDR6Actual > start & is.na(PDR5Actual) ~ "Missing",
...
Enrollment11 <- Enrollment10%>%
      ...
      LENAD3DiaryActual > start & is.na(PDR6Actual) ~ "Missing",
...
Enrollment12 <- Enrollment11%>%
      ...
      LENAD4DiaryActual > start & is.na(LENAD3DiaryActual) ~ "Missing",
...
Enrollment14$Status <- factor(Enrollment14$Status, levels = c("Complete", "Delinquent","Missing", "NA", "On-Track", "Warning"))
Enrollment14$Status <- forcats::fct_relevel(Enrollment14$Status, "NA", after = Inf)
Enrollment14$Status <- forcats::fct_relevel(Enrollment14$Status, "On-Track", after = 1)
Enrollment14$Status <- forcats::fct_relevel(Enrollment14$Status, "Warning", after = 2)
Enrollment14$Status <- forcats::fct_relevel(Enrollment14$Status, "Missing", after = 4)
...
scale_fill_manual(values = c("#bdbdbd","#fb9a99","#e31a1c","#ffff99", "#1f78b4", "#33a02c" ), labels = c("NA", "Missing","Delinquent","Warning", "On-Track", "Complete")) #Manually assigns colors
...
Wave1Counts <- kable(Wave1,
        col.names = c("Complete","On-Track", "Warning","Delinquent","Missing", "NA")) %>%
