...
CoachImp <- data.frame("RSID" = CoachImp$RSID,
                        ...
                        "CoachImpCompletion" = CoachImp$CoachImpCompletion)
...
par(mar = c(4, 4, .2, .1))
ConsentComp<- (Enrollment$ConsentCompletion == "Complete")
ConsentComp <- length(grep("TRUE", ConsentComp))
ConsentInProg<- (is.na(Enrollment$ConsentCompletion))
ConsentInProg <- length(grep("TRUE", ConsentInProg))
PostConsentComp<- (Enrollment$ConsentCompletion == "Complete" & Enrollment$SubInfoCompletion == "Complete")
PostConsentComp<- length(grep("TRUE", PostConsentComp))
PostConsentInProg<- (Enrollment$ConsentCompletion == "Complete" & is.na(Enrollment$SubInfoCompletion))
PostConsentInProg <- length(grep("TRUE", PostConsentInProg))
Wv1Comp<- (Enrollment$ConsentCompletion == "Complete" & Enrollment$LENAD2DiaryCompletion == "Complete")
Wv1Comp <- length(grep("TRUE", Wv1Comp))
Wv1DropOut <- (Enrollment$DropOutStage == 1)
Wv1DropOut <- length(grep("TRUE", Wv1DropOut))
Wv1InProg<- (is.na(Enrollment$LENAD2DiaryCompletion) & Enrollment$ConsentCompletion == "Complete")
Wv1InProg <- length(grep("TRUE", Wv1InProg))
IntvComp<- (Enrollment$IntvCompletion == "Complete")
IntvComp <- length(grep("TRUE", IntvComp))
IntvInProg<- (is.na(Enrollment$IntvCompletion) & Enrollment$LENAD2DiaryCompletion == "Complete")
IntvInProg <- length(grep("TRUE", IntvInProg))
IntvDropOut <- (Enrollment$DropOutStage == 2)
IntvDropOut <- length(grep("TRUE", IntvDropOut))
Wv2Comp<- (Enrollment$LENAD4DiaryCompletion == "Complete")
Wv2Comp <- length(grep("TRUE", Wv2Comp))
Wv2InProg<- (is.na(Enrollment$LENAD4DiaryCompletion) & Enrollment$Survey3Completion == "Complete")
Wv2InProg <- length(grep("TRUE", Wv2InProg))
Wv2DropOut <- (Enrollment$DropOutStage == 3)
Wv2DropOut <- length(grep("TRUE", Wv2DropOut))
Wv3DropOut <- (Enrollment$DropOutStage == 4)
Wv3DropOut <- length(grep("TRUE", Wv3DropOut))
TotalInProg<- sum(PostConsentInProg,Wv1InProg,IntvInProg, Wv2InProg)
...
scale_fill_manual(values = c("#e31a1c","#737373" ,"#bdbdbd", "#1f78b4","#33a02c" #"#ffff99"
                             ), labels = c("NA", "On-Track", "Opt-Out",  #"Warning",  
                                            "Delinquent", "Complete")) #Manually assigns colors
