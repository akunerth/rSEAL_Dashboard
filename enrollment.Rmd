...
#Classify Tests
ConsentCompletion <- Enrollment$ConsentCompletion <- ifelse(Enrollment$StaffSignature > "2020-07-01", "Complete", "Incomplete")
...
PreStudyComp<- (ConsentCompletion == "Complete")
PreStudyComp <- length(grep("TRUE", PreStudyComp))

PreStudyInProg<- (is.na(ConsentCompletion))
PreStudyInProg <- length(grep("TRUE", PreStudyInProg))
...
<div class="btn-group btn-group-justified">
 <div class="btn-group">
<a href="#" class="btn btn-primary">**PreStudy Complete**<br>`r PreStudyComp`</a>
  </div>
  
  <div class="btn-group">
<a href="#" class="btn btn-primary">**PreStudy In Progress**<br>`r PreStudyInProg`</a>
  </div>
...
<div class="btn-group btn-group-justified">
 <div class="btn-group">
<a href="#" class="btn btn-default">**Wave 1 Complete**<br>`r Wv1Comp`</a>
  </div>
  
  <div class="btn-group">
<a href="#" class="btn btn-default">**Wave 1 In Progress**<br>`r Wv1InProg`</a>
  </div>
...
<div class="btn-group btn-group-justified">
 <div class="btn-group">
<a href="#" class="btn btn-danger">**Wave 3 Complete**<br>TBD</a>
  </div>
 <div class="btn-group">
<a href="#" class="btn btn-danger">**Wave 3 In Progress**<br>TBD</a>
  </div>
</div>
</div>
<br>
...
Test0 <- today - Enrollment$InformedConsent
...
Enrollment1 <- Enrollment %>%
      select(RSID:ConsentCompletion) %>%
      mutate(
      PreStudyStatus = case_when(
      Test0 > 5 & is.na(Enrollment$StaffSignature) ~ "Delinquent",
      Test0 <= 5 & Test >= 2 & is.na(Enrollment$StaffSignature) ~ "Warning",
      Test0 < 2 & is.na(Enrollment$StaffSignature) ~ "On-Track",
      Enrollment$StaffSignature > start       ~ "Complete",
      TRUE                      ~ "NA"
      )
     )
Enrollment2 <- Enrollment1 %>%
      select(RSID:PreStudyStatus) %>%
      mutate(
...
Enrollment12 <- Enrollment11%>%
      ...
      Test10 > 14 & is.na(LENAD3DiaryActual) ~ "Delinquent",
      Test10 <= 14 & Test >= 7 & is.na(LENAD3DiaryActual) ~ "Warning",
...
Enrollment13 <- Enrollment12%>%
      ...
      Test11 > 7 & is.na(LENAD4DiaryActual) ~ "Delinquent",
      Test11 <= 7 & Test >= 3 & is.na(LENAD4DiaryActual) ~ "Warning",
...
Enrollment14 <- gather(Enrollment13, "Instrument", "Status", PreStudyStatus, Surv1Status, Surv2Status,PDR2Status, PDR3Status,LENAD1DiaryStatus,LENAD2DiaryStatus,Surv3Status, Surv4Status,PDR5Status, PDR6Status,LENAD3DiaryStatus,LENAD4DiaryStatus,)
library(ggplot2) #load ggplot2 library
Enrollment14$Status <- factor(Enrollment14$Status, levels = c("Complete", "Delinquent", "NA"))
Enrollment14$Cases <- factor(Enrollment14$RSID) # Create a categorical variable
Enrollment14$Instrument <- factor(Enrollment14$Instrument, levels = c("PreStudyStatus", "Surv1Status","Surv2Status","PDR2Status","PDR3Status", "LENAD1DiaryStatus","LENAD2DiaryStatus", "Surv3Status","Surv4Status","PDR5Status","PDR6Status", "LENAD3DiaryStatus","LENAD4DiaryStatus")) # Create categorical variable
p <- ggplot(data = Enrollment14, aes(x=Instrument, fill= forcats::fct_rev(Status))) + geom_bar() + # Creates stacked bar chart
scale_x_discrete(labels=c("PreStudy","Survey 1", "Survey 2", "PDR2", "PDR3", "LENA D1 Diary", "LENA D2 Diary", "Survey 3", "Survey 4", "PDR5", "PDR6", "LENA D3 Diary", "LENA D4 Diary")) +
scale_fill_manual(values = c("darkgrey","brown1","darkgreen" ), labels = c("NA", "Delinquent","Complete")) #Manually assigns colors
...
library(kableExtra)
rownames(Wave1) = c("PreStudy", "Survey 1", "Survey 2", "PDR2", "PDR3", "LENA D1 Diary", "LENA D2 Diary", "Survey 3", "Survey 4", "PDR5", "PDR6", "LENA D3 Diary", "LENA D4 Diary")
